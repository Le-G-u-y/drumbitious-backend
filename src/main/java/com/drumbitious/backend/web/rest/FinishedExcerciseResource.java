...
/**
 * REST controller for managing {@link com.drumbitious.backend.domain.FinishedExcercise}.
 */
@RestController
@RequestMapping("/api")
public class FinishedExcerciseResource {
    ...
    private final FinishedExcerciseQueryService finishedExcerciseQueryService;

    public FinishedExcerciseResource(FinishedExcerciseService finishedExcerciseService, FinishedExcerciseQueryService finishedExcerciseQueryService) {
        this.finishedExcerciseService = finishedExcerciseService;
        this.finishedExcerciseQueryService = finishedExcerciseQueryService;
    }
    ...
    public ResponseEntity<List<FinishedExcerciseDTO>> getAllFinishedExcercises(FinishedExcerciseCriteria criteria) {
        log.debug("REST request to get FinishedExcercises by criteria: {}", criteria);
        List<FinishedExcerciseDTO> entityList = finishedExcerciseQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
    * {@code GET  /finished-excercises/count} : count all the finishedExcercises.
    *
    * @param criteria the criteria which the requested entities should match.
    * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
    */
    @GetMapping("/finished-excercises/count")
    public ResponseEntity<Long> countFinishedExcercises(FinishedExcerciseCriteria criteria) {
        log.debug("REST request to count FinishedExcercises by criteria: {}", criteria);
        return ResponseEntity.ok().body(finishedExcerciseQueryService.countByCriteria(criteria));
    }
