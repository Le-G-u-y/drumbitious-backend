...
export class PlanUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.exerciseConfigService
      .query()
      .pipe(
        filter((mayBeOk: HttpResponse<IExerciseConfig[]>) => mayBeOk.ok),
        map((response: HttpResponse<IExerciseConfig[]>) => response.body)
      )
      .subscribe((res: IExerciseConfig[]) => (this.exerciseconfigs = res), (res: HttpErrorResponse) => this.onError(res.message));
  }
  ...
  updateForm(plan: IPlan) {
    this.editForm.patchValue({
      ...
      exerciseConfigId: plan.exerciseConfigId
    });
  }
  ...
  private createFromForm(): IPlan {
    return {
      ...
      exerciseConfigId: this.editForm.get(['exerciseConfigId']).value
    };
  }
  ...
  trackExerciseConfigById(index: number, item: IExerciseConfig) {
    return item.id;
  }
}
