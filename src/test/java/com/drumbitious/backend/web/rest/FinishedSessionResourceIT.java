...
public class FinishedSessionResourceIT {
    ...
    @Test
    @Transactional
    public void getAllFinishedSessionsByExerciseIsEqualToSomething() throws Exception {
        // Initialize the database
        finishedSessionRepository.saveAndFlush(finishedSession);
        FinishedExercise exercise = FinishedExerciseResourceIT.createEntity(em);
        em.persist(exercise);
        em.flush();
        finishedSession.addExercise(exercise);
        finishedSessionRepository.saveAndFlush(finishedSession);
        Long exerciseId = exercise.getId();

        // Get all the finishedSessionList where exercise equals to exerciseId
        defaultFinishedSessionShouldBeFound("exerciseId.equals=" + exerciseId);

        // Get all the finishedSessionList where exercise equals to exerciseId + 1
        defaultFinishedSessionShouldNotBeFound("exerciseId.equals=" + (exerciseId + 1));
    }
