...
public class PlanResourceIT {
    ...
    @Test
    @Transactional
    public void getAllPlansByExerciseConfigIsEqualToSomething() throws Exception {
        // Initialize the database
        planRepository.saveAndFlush(plan);
        ExerciseConfig exerciseConfig = ExerciseConfigResourceIT.createEntity(em);
        em.persist(exerciseConfig);
        em.flush();
        plan.setExerciseConfig(exerciseConfig);
        planRepository.saveAndFlush(plan);
        Long exerciseConfigId = exerciseConfig.getId();

        // Get all the planList where exerciseConfig equals to exerciseConfigId
        defaultPlanShouldBeFound("exerciseConfigId.equals=" + exerciseConfigId);

        // Get all the planList where exerciseConfig equals to exerciseConfigId + 1
        defaultPlanShouldNotBeFound("exerciseConfigId.equals=" + (exerciseConfigId + 1));
    }
